...
export async function PUT(request: NextRequest, context: RouteContext) {
  try {
    ...
    const agentIndex = agentConfigs.findIndex(a => a.id === id)
    ...
    const updatedAgent = {
      ...agentConfigs[agentIndex],
      ...body,
      id: id // Préserver l'ID original
    }
    
    agentConfigs[agentIndex] = updatedAgent
    ...
    return NextResponse.json(
      { success: false, error: 'Erreur lors de la mise à jour de l\'agent' },
      { status: 500 }
    )
  }
}
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  try {
    ...
    const deletedAgent = agentConfigs.splice(agentIndex, 1)[0]
    ...
    return NextResponse.json(
      { success: false, error: 'Erreur lors de la suppression de l\'agent' },
      { status: 500 }
    )
  }
}